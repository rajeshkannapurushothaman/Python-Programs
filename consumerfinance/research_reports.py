# -*- coding: utf-8 -*-
import scrapy
class ResearchReportsItem(scrapy.Item):
...
class ResearchReportsSpider(scrapy.Spider):
        ...
        try:
            if response.status == 200:
                for artical in response.xpath('//article'):
                    ...
                    topics = []
                    for aEle in artical.xpath('./descendant::ul[@aria-label="Topics"]/li/a'):
                        topic = aEle.xpath('normalize-space(./text()[last()])').get()
                        topics.append(topic)
